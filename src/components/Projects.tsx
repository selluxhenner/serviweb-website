...
function SpotlightCard({ children, href, to, className, style, onClick }: {
  ...
  const spotlight = useMotionTemplate`radial-gradient(220px circle at ${mx}px ${my}px, rgba(59,130,246,0.14), transparent 70%)`
...
const WEBSITES = [
  ...
  },
  {
    ...
    accentColor: 'rgba(234,179,8,0.55)',
  },
  {
...
const APPS: {
  ...
  tags: string[]; image: string; accentColor: string; initBg: string
...
}[] = [
  {
    ...
    image: kantiImg, accentColor: 'rgba(234,179,8,0.60)', initBg: 'linear-gradient(135deg,#EAB308,#F97316)',
  ...
  },
  {
    ...
    image: schoolImg, accentColor: 'rgba(16,185,129,0.60)', initBg: 'linear-gradient(135deg,#10B981,#06B6D4)',
  ...
  },
  {
    ...
    image: dailyGoalsImg, accentColor: 'rgba(236,72,153,0.55)', initBg: 'linear-gradient(135deg,#EC4899,#8B5CF6)',
      ...
      >
        ...
        <div className="h-44 relative overflow-hidden rounded-t-2xl">
          <img
            ...
            loading="lazy"
          />
          <div className="absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-[rgba(13,21,38,0.85)] to-transparent pointer-events-none" />
          <div className="absolute top-2.5 right-2.5 text-[10px] font-semibold px-2 py-0.5 rounded-full border border-white/10 text-white/70 bg-black/40"
            style={{ backdropFilter: 'blur(6px)' }}>
            {p.category}
          </div>
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"
            style={{ background: 'rgba(7,11,20,0.6)' }} aria-hidden="true">
        ...
        <div className="p-5">
          ...
          <div className="flex flex-wrap gap-1.5">
            {p.features.slice(0, 3).map((f) => (
              <span key={f} className="text-[10px] text-slate-500 px-2 py-0.5 rounded-md bg-slate-800/60 border border-slate-700/40">
                {f}
              </span>
            ))}
          </div>
          <p className="mt-3 text-[11px] text-blue-400/70 font-medium">{p.url}</p>
        </div>
      </SpotlightCard>
    </motion.div>
  )
...
const AppBadge = ({ label }: { label: string }) => (
  <span className="text-[10px] font-semibold px-2.5 py-1 rounded-lg border border-blue-500/20 bg-blue-500/8 text-blue-300">
    {label}
  </span>
)
    ...
    <motion.div variants={gridItem}>
      ...
      >
        ...
        <div className="h-40 relative overflow-hidden rounded-t-2xl">
          ...
          <div className="absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-[rgba(13,21,38,0.85)] to-transparent pointer-events-none" />
          {p.store.comingSoon && (
            <div className="absolute top-2.5 right-2.5 px-2 py-0.5 rounded-full text-[10px] font-bold text-amber-300 border border-amber-400/30 bg-amber-400/10">
              Coming Soon
            </div>
          )}
        ...
        <div className="p-5">
          <h3 className="font-bold text-white text-base leading-tight mb-0.5">{p.name}</h3>
          <p className="text-xs text-slate-500 mb-2">{p.subtitle}</p>
          <p className="text-sm text-slate-400 leading-relaxed mb-3">{p.desc}</p>
          ...
          <div className="flex items-center gap-2">
            {p.store.comingSoon ? (
              <span className="text-xs text-slate-600 italic">App Store & Play Store – demnächst</span>
            ) : (p.store.ios ? (
              <>
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800/60 border border-slate-700/40">
                  <svg className="w-3.5 h-3.5 text-slate-400" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                    <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
                  </svg>
                  <span className="text-[10px] text-slate-400 font-medium">App Store</span>
                </div>
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800/60 border border-slate-700/40">
                  <svg className="w-3.5 h-3.5 text-slate-400" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  ...
                  <span className="text-[10px] text-slate-400 font-medium">Play Store</span>
                </div>
              </>
            ) : null)}
          </div>
    ...
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      <Link to="/projekte/vocafy" className="block group cursor-pointer">
        <motion.div
          ...
          style={{ background: 'rgba(13,21,38,0.72)', border: '1px solid rgba(59,130,246,0.15)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }}
        >
...
export default function Projects() {
  ...
  return (
    <section id="projekte" className="relative py-24 md:py-32" style={{ background: '#07101E' }}>
      <div className="absolute top-0 inset-x-0 h-px pointer-events-none"
        style={{ background:'linear-gradient(90deg,transparent,rgba(59,130,246,0.18),transparent)' }} aria-hidden="true" />
      <div className="absolute bottom-0 right-0 w-[500px] h-[400px] pointer-events-none"
        style={{ background:'radial-gradient(ellipse,rgba(59,130,246,0.06) 0%,transparent 70%)', filter:'blur(80px)' }} aria-hidden="true" />
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        <motion.div
          ...
          className="mb-10"
        >
          <p className="text-xs font-semibold tracking-widest text-slate-500 uppercase mb-4">Meine Projekte</p>
            ...
            >
              ...
              <span className="relative z-10 flex items-center gap-2">
                ...
                <span className={`text-xs px-1.5 py-0.5 rounded-md font-bold ${
                  activeTab === tab.id ? 'bg-white/20' : 'bg-slate-700/60'
                }`}>
                  {tab.count}
                </span>
              </span>
        ...
        >
          <Link to="/projekte">
            <motion.span
              ...
              whileTap={{ scale:0.97 }}
              className="inline-flex items-center gap-2.5 px-7 py-3.5 rounded-2xl text-sm font-semibold text-slate-300 hover:text-white cursor-pointer transition-colors"
              style={{ border:'1px solid rgba(59,130,246,0.25)', background:'rgba(59,130,246,0.07)', display:'inline-flex' }}
            >
      ...
      <div className="absolute bottom-0 inset-x-0 h-px pointer-events-none"
        style={{ background:'linear-gradient(90deg,transparent,rgba(59,130,246,0.12),transparent)' }} aria-hidden="true" />
    </section>
  )
}

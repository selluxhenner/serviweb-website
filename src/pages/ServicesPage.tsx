      ...
      <div className="relative">
        ...
        {SERVICES.map((s, i) => (
          <section key={s.id} id={s.id}
            ...
            style={{ background: i % 2 === 0 ? '#F1F5F9' : '#FFFFFF' }}>
            ...
            <div className="relative max-w-6xl mx-auto px-6">
              <div className={`grid grid-cols-1 lg:grid-cols-2 gap-14 items-start ${i % 2 !== 0 ? 'lg:flex-row-reverse' : ''}`}>
                ...
                <motion.div initial={{ opacity:0, x: i % 2 === 0 ? -24 : 24 }}
                  ...
                  <div className="flex items-center gap-3 mb-5">
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center"
                      ...
                      <svg className="w-6 h-6" style={{ color:'#2563EB' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.7}>
                        <path strokeLinecap="round" strokeLinejoin="round" d={s.icon} />
                      </svg>
                    </div>
                    <div>
                      <div className="text-xs font-semibold uppercase tracking-wider" style={{ color:'#2563EB' }}>{s.tagline}</div>
                    </div>
                  </div>

                  <h2 className="text-3xl md:text-4xl font-black mb-4" style={{ color: '#1E3A8A' }}>{s.label}</h2>
                  <p className="leading-relaxed mb-6" style={{ color: '#64748B' }}>{s.desc}</p>

                  <div className="flex flex-wrap gap-2 mb-8">
                    {s.tags.map(t => (
                      <span key={t} className="px-2.5 py-1 rounded-lg text-xs font-medium border"
                        style={{ background: 'rgba(37,99,235,0.06)', color: '#2563EB', borderColor: 'rgba(37,99,235,0.2)' }}>{t}</span>
                    ))}
                  </div>

                  <div className="rounded-2xl p-6 mb-6"
                    style={{ background: i % 2 === 0 ? '#FFFFFF' : '#F1F5F9', border:'1px solid #E2E8F0' }}>
                    <p className="text-xs font-semibold uppercase tracking-wider mb-4" style={{ color:'#2563EB' }}>Inbegriffen</p>
                    <ul className="space-y-2.5">
                      {s.included.map(item => (
                        <li key={item} className="flex items-start gap-2.5 text-sm" style={{ color: '#0F172A' }}>
                          <Check />
                          {item}
                        </li>
                      ))}
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 text-sm" style={{ color: '#64748B' }}>
                      ...
                      <span>Lieferzeit: <strong style={{ color: '#0F172A' }}>{s.timeline}</strong></span>
                    </div>
                    <Link to="/kontakt">
                ...
                <motion.div initial={{ opacity:0, x: i % 2 === 0 ? 24 : -24 }}
                  ...
                  <p className="text-xs font-semibold uppercase tracking-wider mb-6" style={{ color:'#2563EB' }}>So läuft es ab</p>
                    ...
                    {s.process.map((step, si) => (
                      <motion.div key={step.step}
                        ...
                        <div className="flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center text-xs font-black"
                          style={{ background:'rgba(37,99,235,0.1)', color:'#2563EB' }}>{step.step}</div>
                        <div>
                          <p className="font-bold text-sm mb-1" style={{ color: '#1E3A8A' }}>{step.title}</p>
        ...
        <section className="py-20" style={{ background:'#F1F5F9' }}>
          <div className="max-w-4xl mx-auto px-6 text-center">
            <motion.div initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }}
              ...
              <h2 className="text-3xl md:text-4xl font-black mb-4" style={{ color: '#1E3A8A' }}>Preise & Pakete</h2>
              <p className="mb-8 max-w-md mx-auto leading-relaxed" style={{ color: '#64748B' }}>
                Transparente Preise ohne versteckte Kosten. Pakete ab CHF 890.–
              </p>
              <div className="flex flex-wrap gap-4 justify-center">
                <Link to="/#preise">
                  <motion.span whileHover={{ y:-2 }} whileTap={{ scale:0.97 }}
                    className="inline-flex items-center gap-2.5 px-7 py-3.5 rounded-2xl text-sm font-semibold cursor-pointer"
